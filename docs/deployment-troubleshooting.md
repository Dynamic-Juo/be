...
## 영속 접수 잠금과 자동 배포 수렴 충돌

31ad2a5 자동 배포에서 이미지·journal은 committed였지만 신규 접수가 draining으로 남았다. 읽기 전용 root를 구성하면서 admission 파일을 결과 영속 볼륨으로 옮겨, 컨트롤러가 기대하는 재생성 시 잠금 초기화가 되지 않은 운영 설정 오류였다. 정확한 이미지·state/journal을 대조해 인증 resume와 recover로 복구했고 차단된 시도는 증거를 보관한 후 성공 상태로 조정했다. 다음 배포부터 잠금은 UID 10001의 전용 1MiB tmpfs(`/var/lib/deepcheck`)를 사용하고 결과·quota만 영속 볼륨에 둔다. 프로세스 재시작에서는 tmpfs가 유지되고 컨테이너 재생성에서는 비워지는 계약이다. watcher의 실패 차단을 단순 삭제해서 우회하지 않았다.
