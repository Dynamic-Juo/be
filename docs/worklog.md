...
## 2026-09-16 주장 추출 프롬프트 — 문장 통째가 아니라 사실 단위로 분리

**발견.** 사용자가 "주장을 의미 있게 못 나눈다"고 지적했다. 실측 결과를 다시 보니 추출된 "주장" 11건 중 5건이 사실 2~3개를 담은 복합 문장이었다. 예: "버팀목 대출과 디딤돌 대출의 금리를 동결하고 저소득층 알뜰교통카드는 할인을 확대합니다"가 한 항목.

**원인.** 프롬프트 규칙 3이 "text는 발언 전문의 연속된 구절을 그대로 복사"와 "독립적으로 검증할 수 있는 최소 주장"을 동시에 요구했는데, 모델은 앞쪽을 우선해 문장 전체를 복사했다. "그대로 복사"는 환각 방지 원문 대조의 핵심이라 없앨 수 없다.

**변경.** 규칙 3을 "사실 하나당 항목 하나, text는 그 사실이 드러나는 최소한의 연속 구절"로 바꿨다. 연속 구절이면 원문 대조를 통과하므로 안전장치는 그대로다. 사실을 정하는 조건·부정·비교·범위는 함께 두도록 제약을 남겼다. `PROMPT_VERSION` 2026-09-16.1.

**실측(같은 YTN 추석 영상 전사, DeepSeek).** 11건 → 16건, 전부 원문 대조 통과. 복합 문장 5건이 각각 2건씩 갈렸고, "버팀목 대출과 디딤돌 대출 금리 동결"은 같은 사실이라 갈리지 않았다 — 모델이 그 판단을 알아서 했다.

**왜 판정에 중요한가.** 복합 주장은 근거가 그중 하나만 확인해도 `partial` → 근거 부족이다. 나누면 확인된 쪽은 일치, 안 된 쪽은 부족으로 각각 나간다.

**남은 것.** 앵커 도입부 요약("대책을 내놓았다", "논의했다")처럼 본문이 구체화하는 메타 주장이 여전히 뽑힌다. M-04가 "모두 검증"이라 버릴 수는 없고 우선순위(등장 순서보다 구체성)로 뒤로 보내야 한다. STT 오탈자("바늘"=마늘, "투익류면제"=통행료 면제)가 검색어에 그대로 들어가는 문제는 별개다.
