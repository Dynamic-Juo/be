...
높은 점수가 얼굴 crop 프레임에서 나오는지 확인하려고 짠 스크립트에서, 프레임을 crop해서 채점해야 하는데 **원본 전체 화면으로 채점하고 crop 여부만 라벨로 붙였다.** 두 컬럼이 사실 같은 값이었다.

덕분에 우연히 비교가 됐다 — 파이프라인(crop 채점) 결과와 대조하니 같은 프레임이 crop하면 0.997, 전체 화면이면 0.002였다. **오탐이 crop 쪽에서 난다는 뜻이고, 내 원래 가설(얼굴 못 찾은 프레임이 문제)과 정반대다.** 제대로 된 비교를 다시 돌리는 중이다.
