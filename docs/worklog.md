...
## 2026-09-17 검색어 버그 두 건 — 실사용 근거부족의 진짜 원인

제출일 실사용 테스트에서 "영상만 넣으면 맥락 없이 전부 근거부족으로 끝난다"는
문제를 재현·근본 원인까지 추적했다. #1(원문검증)·#5(관련성 필터) 설계 문제가
아니라, 검색어 자체가 아예 잘못 만들어지고 있었다.

1. **해시태그가 이름을 밀어냄** — 쇼츠 제목 끝의 `#백일섭충격사망#마지막인터뷰...`처럼
   공백 없이 이어붙은 해시태그 블록이 `_key_tokens`의 길이 기반 점수에서 압도적으로
   높은 점수를 받아, 검색어 상위 슬롯을 전부 차지했다. 그 결과 실제 검색어에 "백일섭"
   이름이 단 한 번도 들어가지 않은 채 Naver API로 전송되고 있었다(어제 넣은 T-05
   완화책이 오히려 이 버그를 노출시켰다 — 제목을 검색어에 포함하기 전에는 덜 드러났다).
   검색어를 만들 때 제목에서 해시태그 블록을 먼저 제거한다(`_HASHTAG_RE`).
2. **토큰을 너무 많이 AND로 묶으면 Naver가 0건을 반환** — 같은 낱말 집합으로 직접
   실험한 결과, 토큰 3~4개는 결과가 나오고 5~6개는 0건이었다. `_search_query`의
   상한을 6→4로 낮췄다. 또한 순수 길이 기반 점수가 "발표했습니다"·"감소했다고" 같은
   조사 제거로 안 걸러지는 긴 서술어 활용형을 "실업률" 같은 진짜 핵심 명사보다 높게
   매기고 있어서, 서술어 활용형(`_PREDICATE_TAIL_RE`)에는 길이 가산점을 주지 않도록
   고쳤다.
3. **검색어 앵커링 방식 변경** — 어제 제목을 그냥 발화 텍스트에 이어붙여 토큰화했던
   방식(T-05) 대신, `_search_query(claim_text, anchor_text=title)`로 분리해 "발화와
   제목 양쪽에 다 나오는 낱말 → 제목에만 있는 낱말(STT 오인식 보완) → 나머지 발화
   낱말" 순으로 우선순위를 매기도록 재설계했다.

실측(같은 백일섭 영상, 실제 Naver 자격정보로 로컬 재현): 수정 전엔 핵심 주장에
근거 0건 → "관련 자료를 찾지 못함"으로 즉시 종료. 수정 후엔 근거 5건을 찾아
LLM 판정까지 도달했고, "제시된 자료는 전부 이순재의 별세·조문 관련 내용이라
백일섭 본인의 사망은 직접 확인도 반박도 못 한다"는 근거를 실제로 읽고 판단한
사유가 나왔다(`insufficient_reason`도 `no_source`에서 `not_direct`로 바뀜).
사족: 검색 결과 중엔 "이순재 조문 갔다"는 실제 기사 외에도 이 영상 자체를 가짜뉴스로
지목해 반박하는 기사("…대놓고 '가짜뉴스' 충격")도 있었다 — LLM 판정이 이걸 인용해
`REFUTED`까지 가는지는 이번 로컬 검증 범위 밖이라 확인하지 못했다.

주의: `.env`를 로컬에서 `source`만 하고 `set -a`로 export하지 않으면 `os.environ`에
안 실려서 자격정보가 있어도 "없다"고 로그가 난다(config.py는 dotenv 자동 로드를
안 한다) — 이번에 두 번 헛짚었다.

pytest 781 passed.

## 2026-09-17 프로덕션 evidence_providers가 위키백과를 계속 쓰고 있었음

- 코드 기본값(`config.py`)은 이미 위키백과를 뺐지만, 세 곳에 하드코딩된 fallback이
  남아 실제로는 안 반영되고 있었다: `docker-compose.yml`·`deploy/compose.development.yml`의
  `${DEEPCHECK_EVIDENCE_PROVIDERS:-...}` 기본값(리포에서 수정·커밋함), 그리고
  결정적으로 서버의 `ops-secrets/runtime.env`가 `DEEPCHECK_EVIDENCE_PROVIDERS=...,wikipedia,wikipedia_en`로
  명시적 오버라이드를 갖고 있어서 컨테이너에 그대로 주입되고 있었다.
- `ops-secrets/runtime.env` 수정은 프로덕션 시크릿 파일 쓰기라 보안 분류기가
  에이전트 직접 실행을 막았다 — 사용자에게 정확한 한 줄 명령을 전달해 직접
  실행하도록 안내했다(백업은 `runtime.env.bak-<timestamp>`로 미리 떠둠).

## 2026-09-17 검색어에 영상 제목의 고유명사도 포함(T-05 STT 고유명사 불일치 완화)

- STT가 같은 인물명을 문장마다 다르게 인식하는 사례를 실사용에서 확인했다
  (백일섭 → 백일석/101섭, 2026-09-16). 발화 원문은 인용 검증 때문에 고칠 수
  없어 그대로 두고, 대신 `verify_one_claim`의 검색어 구성에 `claim.video_title`도
  함께 넣었다. 영상 제목은 보통 정확한 표기라, 발화 쪽 이름이 잘못 인식돼도
  검색어에 올바른 표기가 섞여 들어갈 확률이 생긴다.
- 근본 해결(STT 결과 자체의 고유명사 정규화)은 아니고 완화책이다 — 제목에
  이름이 없거나 제목도 잘못돼 있으면 효과가 없다. 발화 텍스트를 사후에 고치는
  건 인용 검증 아키텍처와 충돌해 이번에는 손대지 않았다.
- pytest 778 passed, 2 skipped.

## 2026-09-17 CI/CD 완전 자동화 복구 (저장소 이동으로 어긋난 신원 재승인)

- GitHub 조직이 `Dynamic-Juo` → `chamsae-ai`로 이동하면서, 자동배포 컨트롤러
  (`scripts/auto_deploy.py`)가 매 tick마다 `ApprovalError`로 조용히 실패하고
  있었다(`repository.full_name`이 GitHub API 응답과 설정 파일의 옛 이름이
  달라서). `host-config.json`의 `repository`를 `target_fingerprint` 계산에
  직접 쓰기 때문에, 이 필드를 고치면 기존 `state.json`/`journal.json`과
  fingerprint가 어긋나 fail-closed로 거부되는 구조였다.
- `repository`·`bootstrap_active`를 오늘 실제 배포된 이미지(커밋 `ef01005`,
  digest로 검증)로 갱신하고, 옛 신원을 가진 `state.json`·`journal.json`·
  `image.env`·`auto-attempt.json`을 전부 백업 후 제거해 `initialize`로
  재부트스트랩했다. 각 파일이 막힌 이유를 하나씩 진단해 총 4개의 서로 다른
  잔여 파일을 순서대로 정리해야 했다.
- 재부트스트랩 후 `tick()`을 직접 호출해 `{'status': 'unchanged'}`로 정상
  동작 확인. 이어서 실제 커밋(`a275e28`)을 push해 CI 성공 후 **완전 자동으로**
  감지·배포되는 것까지 확인했다(컨테이너 리비전 라벨이 정확히 그 커밋과
  일치, 수동 개입 없음). 다른 7개 서비스는 영향 없이 계속 Up 상태였다.
- 백업 파일들은 각 원래 위치에 `*.bak-<타임스탬프>`로 남아 있다.

## 2026-09-17 주장 추출: 항목 하나가 원문과 안 맞아도 전체를 버리지 않음

- 클러스터링 규칙 배포 직후 실사용(연합뉴스 예산 관련 영상)에서 "LLM 주장 5번이
  발언 전문에 없다"로 주장 추출 전체가 실패하는 것을 확인했다. 전날 예능 영상
  건("LLM 주장 1번이 발언 전문에 없다")과 같은 유형으로, 같은 문제를 이틀 연속
  실사용에서 겪었다.
- `extract_claims_llm`의 원문 대조 실패를 `ClaimExtractionError`(배치 전체 중단)
  대신 해당 항목만 버리고 계속하도록 바꿨다. 문맥(context)이 원문에 없는 경우도
  같은 이유로 주장 자체는 살리고 문맥만 비운다. 응답이 비어있지 않은데 전부
  걸러져 최종 claims가 빈 리스트면(정상적인 "주장 없음"과 구분해) 여전히
  ClaimExtractionError로 처리한다.
- 회귀 테스트 갱신: `test_LLM이_원문에_없는_문장을_지어내면_그_항목만_버려진다`,
  `test_context_outside_transcript_is_ignored_but_claim_survives`.
- pytest 777 passed, 2 skipped. 실모델 재검증은 배포 후 진행한다.

## 2026-09-17 주장 추출 프롬프트에 핵심 사실 단위 클러스터링 규칙 추가

- 같은 백일섭 오보 영상 실사용에서, 하나의 사망 사실이 13개의 개별 주장 카드로
  쪼개지는 것을 확인했다(도입 서사문 1건, 핵심 사실 1건, 나머지 11건은 정황 묘사·
  제3자 인용·원인 추정 등 같은 사실의 세부 서술). 각 카드가 독립적으로 검색되다
  보니 핵심 사실 카드를 제외한 나머지는 검색어 자체가 성립하지 않아 대부분
  근거 부족으로 끝났고, 화면도 서사가 뚝뚝 끊긴 느낌을 줬다.
- `deepcheck/prompts.py`의 `EXTRACT_SYSTEM`에 규칙 3번을 추가했다. 하나의 사건·
  인물을 여러 문장이 나눠 설명하면 외부 근거로 독립적으로 확인 가능한 핵심 사실
  문장 하나만 고르고, 배경·정황·인용·원인 추정처럼 그 자체로는 검색해 확인할
  대상이 없는 문장은 주장으로 만들지 않도록 명시했다. 서로 다른 인물·사건·수치는
  이 규칙으로 합치지 않는다는 것도 명시해, 여러 개별 통계를 다루는 뉴스에서
  과도하게 뭉치는 걸 막으려 했다.
- `PROMPT_VERSION`을 `2026-09-11.1` → `2026-09-17.1`로 올렸다.
- **실모델 평가는 하지 않았다.** DeepSeek API 키가 이 작업 환경에 없어 실제로
  이 규칙을 지키는지, 혹은 반대로 서로 다른 사실까지 과도하게 합치는 회귀를
  만드는지는 확인하지 못했다. 회귀 테스트는 "LLM이 이미 잘 뭉쳐서 응답했을 때
  우리 코드가 그걸 그대로 받아들이는지"만 검증한다(`tests/test_prompt_contract.py`).
  배포 후 같은 백일섭 영상 등으로 실제 추출 결과를 재확인해야 한다.
- pytest 777 passed, 2 skipped.
...
- 팀장님은 Vercel 서버의 기존 키 값을 새 이름으로 옮기고 갱신된 Function 예제를 반영해 재배포한다. 프론트/Vercel은 이번 작업에서 변경하지 않았다.

## 2026-09-15 분석 접수 한도 상향

- 사용자 요청으로 전체 분석 접수 30→300회/UTC 일자, 동일 IP 3→60회/시간으로 상향했다. 동시 처리·자원·봇 확인·조회 한도는 유지한다. 새 이미지 배포 전에는 코드 변경과 운영 반영을 구분한다.
