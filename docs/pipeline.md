...
확정 T-02에 따라 기본값은 `off`이며 STT를 실행한다. 명시적으로 `manual` 또는 `any`를 선택한 실험에서만 자막을 우선 파싱하고, 사용할 자막이 없으면 STT로 넘어간다. 자막을 사용한 결과에는 `transcript_source=caption`, STT 결과에는 `stt`를 남긴다. 자막이 더 정확하다고 가정하지 않는다.
...
당시 `tiny` 표본의 수치·고유명사 오류는 후속 주장 검증을 오염시킬 수 있다. 이 표만으로 모델의 일반 한국어 성능을 확정하거나 기능을 끄기로 결정하지 않는다. 자막도 오기·발언 불일치가 있을 수 있으며, 현행 MVP 기본 STT 검수 대신 자막 실험 결과를 사용해서는 안 된다.
