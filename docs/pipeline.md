...
flowchart TD
    ...
    Q2 -->|"없음"| HOLD
    Q2 -->|"있음"| LLM["LLM 판정<br/>주장 + 근거 원문 제시"]
...
그래서 LLM에게 판정과 함께 **근거 원문에서 그대로 발췌한 문장**을 제출하게 하고, 그 문장이 실제 근거에 있는지 서버가 문자열로 대조한다. 없으면 판정을 통째로 버리고 `근거 부족`으로 강등한다. 생성의 위험을 검증으로 막는 구조라, 모델이 지어내도 코드가 잡는다.
