...
사용자의 여러 건 테스트 요청에 맞춘 코드 기본값이다. 전체 분석 접수는 UTC 일자당 300회, IP 기반 익명 식별자당 시간당 60회다. 같은 공인 IP를 사용하는 팀원들은 한도를 공유한다. 동시 분석 수와 컨테이너 자원 제한은 그대로 유지한다. 봇 확인 시도는 전체 일자당 1,000회·식별자당 분당 10회, 조회는 전체 일자당 20,000회·식별자당 분당 90회다. 429의 `Retry-After`를 따른다. 분석 접수 직후 큐 포화·실패·중복 작업 재사용도 접수 한도를 소비하며 환급하지 않는다.
